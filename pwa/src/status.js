...
async function poll(loop) {
    let json;
    try {
        json = await doRequest('status', document.querySelector('.charging-station-id').value);
    }
    catch (e) {
        loop = false;
        console.error(e);
...
`;
    ...
    const startBtn = document.querySelector('.charging-controls__start');
    startBtn.disabled = (state.balance === 0);
    document.querySelector('.charging-controls__start').addEventListener('click', async (evt) => {
        startBtn.disabled = true;
        await doRequest('start', document.querySelector('.charging-station-id').value);
        await poll(false);
    ...
    document.querySelector('.charging-controls__stop').addEventListener('click', async (evt) => {
        ...
        btn.disabled = true;
        await doRequest('stop', document.querySelector('.charging-station-id').value);
        await poll(false);
        btn.disabled = false;
    });
}
